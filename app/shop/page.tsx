  ...
  return (
    <div className="space-y-10">
      <header className="max-w-2xl">
        ...
        <h1 className="display mt-3 text-4xl sm:text-5xl">Field notes as PDFs.</h1>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        {products.map((p) => (
          <article key={p.slug} className="border border-[var(--color-rule)] p-6">
            <p className="stamp">
              {p.priceLabel} · {p.pages}
            </p>
            <h2 className="display mt-3 text-2xl">{p.title}</h2>
            <p className="mt-3 leading-relaxed text-[var(--color-ink-soft)]">{p.blurb}</p>
            <div className="mt-6">
